...
# wrappers.function_name.run(<inputs>)
...
def test_row2im(demo_fov_row):
    ...
    im, ch_list = wrappers.row2im.run(demo_fov_row)
    ...
    im, ch_list = wrappers.row2im.run(demo_fov_row, ["BF"])
    assert list(
