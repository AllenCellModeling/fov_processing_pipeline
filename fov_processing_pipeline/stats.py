...
def intensity_percentiles_by_channel(im, c, percentile_list=[5, 25, 50, 75, 95]):
    ...
    return dict({'Intensity_Percentiles': percentile_list, 'Ch'+str(c)+'_Percentile_Intensities':results})


def plot_im_percentiles(df, fov_flag =True, save_loc = None, save_flag=False):
    ############################################
    ...
    for row_ind in range(df.shape[0]):
        for ch in range(n_ch):
            vals = df['Ch'+str(ch)+'_Percentile_Intensities'][row_ind]
            print(vals)
            color = palette[ch]
    ...
    if save_flag:
        plt.savefig(save_loc+'_intensity_percentile_stats.png')
